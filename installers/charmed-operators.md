...
In Juju terms, "bootstrap" means "install a Juju controller", which is the is the part of Juju that runs in your cluster and controls the applications. 
...
Otherwise, `<cloud_name>` can be found running `juju status`.

Running `juju status` will now show the Juju Controller in your cloud. 

## 2. Create a model

[Juju models](https://juju.is/docs/olm/models) are a logical grouping of applications and infrastructure that work together to deliver a service or product. In Kubernetes terms, models are namespaces. Models are fundamental concepts in Juju and implement service isolation, access control, repeatability and boundaries.
...
We will name our model `finos-legend-model`
``` bash
