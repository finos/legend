...
start() {
	...
	docker run --name gitlab --env \
		GITLAB_OMNIBUS_CONFIG="external_url 'https://0.0.0.0:6443/'; gitlab_rails['lfs_enabled'] = true; nginx['listen_port'] =443 ; nginx['ssl_certificate'] = '$CERT_FILE' ; nginx['ssl_certificate_key'] = '$KEY_FILE'; letsencrypt['enable'] = false ; gitlab_rails['initial_root_password'] = '$GITLAB_ROOT_PASSWORD';" \
		--env \
